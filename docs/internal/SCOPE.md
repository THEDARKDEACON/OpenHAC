...
- **Example BOMs as library defaults** — in-tree demos (grid-edge RTU, satcom, RS-485, flight-controller seed) may ship SKUs via sidecars / `--catalog-overlay` / vendor cassettes. The compiler must not auto-merge those pinouts or substitute MAX3485 / STM32F407 / AMS1117 when a stranger board misses a row ([COMPONENT_AGNOSTIC_SPEC.md](./COMPONENT_AGNOSTIC_SPEC.md), **UNF-***). Named wrappers (`ESP32_WROOM`) stay; parametric `MCU()` / `RS485_Transceiver()` must not silently become them.
