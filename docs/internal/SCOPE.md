...
**Honest fab claim (Phase-2):** for supported part and board classes, a fabrication compile either emits audited release/Gerber outputs or exits non-zero. Software proof: [PRODUCTION_VALIDATION.md](./PRODUCTION_VALIDATION.md) (`ci_validate_production.py --require-all`). OpenHaC does **not** claim production-ready copper from autoroute alone for HS/RF (see non-goals / **PCB-007**).
...
- **Schematic vs compile SoT** — the native graph remains the compile source of truth. Algorithmic sheets are not a substitute for that graph. When **`schematic_signoff`** is requested, the stamped **review** artifact is `.kicad_sch` ([SCHEMATIC_SIGN_OFF_SPEC.md](./SCHEMATIC_SIGN_OFF_SPEC.md)). Fab packages may still omit the drawing (**FAB-040**). Webview stays optional debug (**FAB-041**).
