...
| **SCH-001** | Done | **``.kicad_sym``** pin **``(at)``** lookup (**``kicad_sym_pinpos``** + **``SymbolPinResolver``**); wires/labels use library offsets when **``Device.kicad_sym``** (etc.) is on **``OPENHAC_KICAD_SYMBOL_DIRS``** / **``KICAD*_SYMBOL_DIR``** (incl. **``KICAD6_SYMBOL_DIR``**); set **``OPENHAC_SCHEMATIC_STUB_ONLY=1``** to force stub-only geometry (debug/deterministic baselines) even if libs are available; set **``OPENHAC_DETERMINISTIC_UUIDS=1``** (or convenience **``OPENHAC_DETERMINISTIC_SCHEMATIC=1``**, or umbrella **``OPENHAC_DETERMINISTIC=1``**) for stable UUIDs so `.kicad_sch` is byte-identical across runs; schematic part/net emission order is stable (sorted by ref / net name) so results do not depend on SKiDL insertion order; emitted schematic coordinates use stable minimal decimal formatting to reduce diff churn; generated `.kicad_pro` JSON uses `sort_keys=True` so it is stable across runs; manifest **``sch_kicad_symbol_dirs_configured``** + **``sch_kicad_symbol_search_paths``** (resolved search directories); manifest **``sch_pin_sort_mode``** = ``alphanumeric_natural``; manifest **``sch001_kicad_sch_suffix``** / **``sch001_kicad_pro_suffix``**; manifest **``str002_schematic_gen_module``** / **``str002_project_gen_module``**; manifest **``sch001_kicad_sym_pinpos_module``** (**``openhac.compiler.kicad_sym_pinpos``**); optional **``{project}.openhac-sch-pinpos-report.json``** (schema **``openhac.sch_pinpos_report.v1``**) summarizes how many endpoints used library pin offsets vs stubs (helps explain mismatches when libs not found); **``EmptySymbolPinResolver``** forces legacy index stubs; **``part_library_name``** for **``SchLib``**; **alphanumeric natural sort** for non-numeric pin numbers on nets (BGA-style order). Golden graph isomorphism vs KiCad pin coords still future. |
