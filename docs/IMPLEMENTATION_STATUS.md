...
| **SCH-005** | Partial | ``register_erc_hook``; **``uart_rx_pullup_erc_hook``**, **``one_wire_pullup_erc_hook``**, **``i2c_pullup_erc_hook``**, **``mdio_pullup_erc_hook``**, **``spi_cs_pullup_erc_hook``**, **``reset_pullup_erc_hook``**, **``swd_swdio_pullup_erc_hook``**, **``jtag_tms_pullup_erc_hook``**, **``missing_footprint_erc_hook``** + tests. Built-in digital rule packs still future. |
...
| **STR-002** | Partial | **``git_branch``**; **``git_describe``** (``git describe --always --dirty``) when the compile cwd is a git worktree; **``compile_pipeline_phases``** (ordered phase names); **``pcb_routing_handoff_schema``** + **``pcb_routing_handoff_json_present``** when routing JSON is emitted; **``bom_alternates_handoff``** when alternates JSON exists; **``release_bundle_suffixes``** (incl. alternates, SI reminder, **``.openhac-bom-expand-hint.md``**, **``.openhac-spice-model-hint.md``**, **``.openhac-autoroute-policy.md``**); **``compile_options``**, **``openhac_env_keys_present``**, **``sch_kicad_symbol_dirs_configured``**, **``pcb_pipeline_handoff``**, **``jit_confidence_histogram``** (when non-empty), **``stackup_json_summaries``** (JSON stackup refs), **``release_zip_path``** when zipping; **``fab_profile``** when set; **``length_match_group_count``**; prior keys (**``compile_strictness``**, hierarchy, spice summary, etc.) unchanged; optional **``.sha256``** sidecar. Human sign-off / full CI golden matrix still future. |
...
| **LIB-002** | Partial | **``part_alternates``** + BOM columns; **``{project}.openhac-bom-alternates.json``** + **``{project}.openhac-bom-expand-hint.md``** (incl. suggested CM workflows) when alternates file exists; manifest **``bom_alternates_handoff``**; release zip suffixes. CM-specific BOM expand/collapse templates still future. |
...
| **PCB-007** | Partial | **``declare_no_autoroute_net()``**; manifest **``no_autoroute_nets``**; **``{project}.openhac-autoroute-policy.md``** (skip layout / no-autoroute nets / diff pairs / **``auto_route=False``**); **``{project}.openhac-pcb-routing-handoff.json``** with stable **``pcb_routing_handoff_schema``** in the manifest. KiCad netclass export still future. |
...
| **SIM-001** | Partial | DB **``spice_include``** / **``spice_subckt``** → BOM fields + **``generate_spice``**; manifest **``spice_annotation_summary``**; **``{project}.openhac-spice-model-hint.md``** (summary + checklist) when annotation summary non-zero. Rich vendor model tables still future. |
...
Closing every **Partial** row to literal **Done** is still a **multi-phase** effort. Recent slices include **manifest handoff** keys (**``compile_pipeline_phases``**, **``pcb_routing_handoff_schema``**, **``bom_alternates_handoff``**, **``fab_profile_geometry_keys``**), **BOM expand-hint** CM workflow notes (**LIB-002**), **SPICE model-hint** checklist (**SIM-001**), and **SCH-005** **``swd_swdio_pullup_erc_hook``** + tests. Earlier in the same program: **ARCH** ([ARCHITECTURE.md](./ARCHITECTURE.md)), **SCH-001** pin ordering, **SCH-004** **``power_net_prefixes``**, **LIB-003** JIT tightening, **LIB-004** strict KiCad without global **``Component``** mutation. Continue subsystem-by-subsystem until **Partial** notes match full **Target state** in the spec (or the spec is narrowed).
...
| Metric | Value | Notes |
|--------|--------|--------|
| **Strict Done** | **~33%** (16 / 48) | Only rows marked **Done** with a spec ID (e.g. **SW-001**, **MFG-001**). |
| **Strict not Done** | **~67%** (32 / 48) | Rows marked **Partial** — real progress, not full spec acceptance. |
| **Weighted (illustrative)** | **~58–67%** | Example: treat each **Partial** as **~0.45–0.5** credit → \((16 + 32 \times 0.5) / 48 \approx 67\%\). Use for **velocity** only; **Partial** is not “half-done” in a formal sense. |

Extra rows (**Layout stub**, **Z3 overlap**, **Board DRC**, etc.) are **not** counted in the 48.

## How many tickets are left?

[PRODUCTION_READINESS_SPEC.md](./PRODUCTION_READINESS_SPEC.md) defines **48** numbered requirement sections (STR-, LIB-, SCH-, SIG-, PCB-, SIM-, MFG-, PWR-, REL-, SW-). The main status table lists **all 48** spec IDs plus a few non-ID rows.

| Bucket | Approx. count | Meaning |
|--------|----------------|--------|
| **Done** (spec IDs only) | **16** | Listed above as **Done** (SW-001–003/005, PWR-001, PCB-005/006, MFG-001/002, STR-001, SCH-004, SIG-003/004, SIM-003, REL-002, PCB-008). |
| **Partial** | **32** | Rows with status **Partial**; full **Target state** in the spec often still open. |
...
**So:** **~32** spec IDs remain **Partial** until notes move to **Done** (or the spec is tightened). Use the spec roadmap table for priority order.
